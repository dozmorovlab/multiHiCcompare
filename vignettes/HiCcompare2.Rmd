...
vignette: >
  %\VignetteIndexEntry{HiCcompare2 Vignette}
...
d <- model.matrix(~factor(hicexp2@metadata$group) + factor(batch))
```
...
```{r}
hicexp2 <- hic_glm(hicexp2, design = d, coef = 2, method = "QLFTest", p.method = "fdr", parallel = FALSE, Plot = TRUE)
```
...
```{r, eval=FALSE}
# use Treat option
hicexp <- hic_glm(hicexp, design = d, coef = 2, method = "Treat", M = 0.5, p.method = "fdr", parallel = FALSE, Plot = FALSE)
```
...
```{r}
manhattan_hicexp(hicexp2, method = "standard")
```
