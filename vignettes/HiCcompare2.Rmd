...
```{r}
data("r1", "r2", "r3", "r4")
hicexp1 <- make_hicexp(r1, r2, r3, r4, groups = c(0, 0, 1, 1), zero.p = 0.8, A.min = 5, filter = TRUE)
hicexp1
```
...
```{r}
hicexp1 <- cyclic_loess(hicexp1, verbose = FALSE, Plot = TRUE, parallel = FALSE, span = 0.2)
```
...
```{r, eval=FALSE}
hicexp2 <- hic_glm(hicexp2, design = d, coef = 2, method = "QLFTest", p.method = "fdr", parallel = FALSE, Plot = FALSE)
```
