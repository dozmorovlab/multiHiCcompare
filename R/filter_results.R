...
#' @param p_cutoff The p-value cutoff you wish to filter
#'    by. Defaults to 0.01.
...
#'     of number of times a region is significant. Defaults
#'     to 0.05.
...
filter_results <- function(hicexp, logfc_cutoff = 1, logcpm_cutoff = 1, p_cutoff = 0.01,
  ...
  if (return_df == "pairedbed") {
    # filter
    res <- res[logCPM >= logcpm_cutoff & abs(logFC) >= logfc_cutoff & p.adj <= p_cutoff, ]
    # reformat
